...
static Gpt_CallBackPtrType g_GptCallBack_Ptr = NULL_PTR;
        ...
        for (ChannelIndex = 0; ChannelIndex < NUM_OF_TIMER_CHANNELS; ChannelIndex++)
        {
            ...
            *(volatile uint32 *)((volatile uint8*)Base_Address + GPTMCFG) = GPT_TIMER_CFG_INDIVIDVAL;
            ...
            case GPT_PREDEF_TIMER_1MS_24BIT:  /* If the user wants to divide the processor frequancey by a prescaler value */
                 ...
                 * Example-1: To generate a 1-second delay using prescaler, T(req)=(2^16)*(Tclock)*Prescaler_value = 65535 * 62.5nsec * 256 = 1.0485 second.
...
void Gpt_StartTimer(Gpt_ChannelType Channel, Gpt_ValueType StartValue)
{
    ...
    case GPT_PREDEF_TIMER_1MS_16BIT: /* |If the user wants to use the same processor frequancey (without prescaler) */
         ...
         * NOTE :This is configured for 16/32Bit Individual Timer configuration.
         */
        *(volatile uint32 *)((volatile uint8 *)Base_Address + GPTMTAILR) = StartValue - 1;
        break;
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_T0(Gpt_CallBackPtrType GptNotification)
{
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_T1(Gpt_CallBackPtrType GptNotification)
{
    if (GptNotification != NULL_PTR)
    {
        g_GptCallBack_Ptr = GptNotification;
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_T2(Gpt_CallBackPtrType GptNotification)
{
    if (GptNotification != NULL_PTR)
    {
        g_GptCallBack_Ptr = GptNotification;
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_T3(Gpt_CallBackPtrType GptNotification)
{
    if (GptNotification != NULL_PTR)
    {
        g_GptCallBack_Ptr = GptNotification;
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_T4(Gpt_CallBackPtrType GptNotification)
{
    if (GptNotification != NULL_PTR)
    {
        g_GptCallBack_Ptr = GptNotification;
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_T5(Gpt_CallBackPtrType GptNotification)
{
    if (GptNotification != NULL_PTR)
    {
        g_GptCallBack_Ptr = GptNotification;
...
Description      : Function callback.
----------------------------------------------------------------------------------------*/
void Gpt_Notification_WT0(Gpt_CallBackPtrType GptNotification)
{
    if (GptNotification != NULL_PTR)
    {
        g_GptCallBack_Ptr = GptNotification;
